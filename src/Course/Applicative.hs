...
replicateA ::
  ...
  -> f a
  -> f (List a)
replicateA n f = sequence $ take n (repeat f)
...
filtering ::
  ...
  -> List a
  -> f (List a)
-- filtering p xs = pick <$> sequence (map p xs)
--   where
--     pick ys = map snd $ filter fst $ ys `zip` xs
filtering _ Nil = pure Nil
filtering p (x :. xs) = let condCons b = if b then (x :.) else id
                        in (condCons <$> (p x)) <*> (filtering p xs)
