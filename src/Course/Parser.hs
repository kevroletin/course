...
character = P $ character'
  where character' (x :. xs) = Result xs x
        character' Nil = ErrorResult UnexpectedEof
...
satisfy ::
  (Char -> Bool)
  -> Parser Char
satisfy cond = character >>= \c -> if cond c then return c
...
natural ::
  Parser Int
natural = list1 digit >>= \x -> case read x of Empty -> failed
                                               Full y -> return y
...
phoneParser ::
  Parser Chars
phoneParser = lift3 (\a b _ -> a :. b) digit phoneBodyParser (is '#')
