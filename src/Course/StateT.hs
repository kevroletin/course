...
instance Monad f => Applicative (StateT s f) where
  ...
  (<*>) ::
   StateT s f (a -> b)
    -> StateT s f a
    -> StateT s f b
  (<*>) (StateT fs) (StateT xs) = StateT $ \s ->
    (fs s) >>= \(f, s') ->
      (xs s') >>= \(x ,s'') -> return (f x, s'')
...
state' ::
  (s -> (a, s))
  -> State' s a
state' f = StateT $ \s -> let (a, s') = f s in Id (a, s')
...
runState' ::
  ...
  -> s
  -> (a, s)
runState' x s = let Id (a, s') = runStateT x s in (a, s')
...
distinctF ::
  ...
  List a
  -> Optional (List a)
distinctF xs = evalT (filtering check xs) S.empty
  where check x = getT >>= \s ->
          putT (S.insert x s) >>
            if x > 100 then (StateT $ \_ -> Empty)
            else return (not $ S.member x s)
...
instance Applicative f => Applicative (OptionalT f) where
  ...
  (<*>) (OptionalT fg) (OptionalT fa) = OptionalT $ applyOptional <$> fg <*> fa
