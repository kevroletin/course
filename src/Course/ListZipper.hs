...
toList ::
  ListZipper a
  -> List a
toList (ListZipper l a r) = reverseConcat l (a :. r)
...
-- [2,1] >1< [3,4,5]
reverseConcat :: List a -> List a -> List a
reverseConcat x y = foldLeft (flip (:.)) y x
...
findLeft f (ListZipper l a r) =
  case break f l of
    (_, Nil) -> IsNotZ
    (ys, x :. xs) -> IsZ $ ListZipper xs x (reverseConcat ys (a :. r))
...
findRight f (ListZipper l a r) =
  case break f r of
    (_, Nil) -> IsNotZ
    (ys, x :. xs) -> IsZ $ ListZipper (reverseConcat ys (a :. l)) x xs
...
moveRight x = IsZ $ moveRightLoop x
...
moveLeftN n x
  | n == 0    = IsZ x
  | n > 0     = bindZipper (moveLeftN (n - 1)) (moveLeft x)
  | otherwise = bindZipper (moveLeftN (n + 1)) (moveRight x)
  where
    bindZipper _ IsNotZ = IsNotZ
    bindZipper f (IsZ t) = f t
...
moveLeftN' n x@(ListZipper l _ r)
  | n > 0 && n > length l = Left (length l)
  | n < 0 && abs n > length r = Left (length r)
  | otherwise = case moveLeftN n x of IsZ y -> Right y
                                      IsNotZ -> Left 0
...
nth ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
nth n x@(ListZipper l _ _) = moveLeftN ((length l) - n) x
...
-- prop> rights (end lz) == Nil
  ...
  ListZipper a
  -> ListZipper a
end (ListZipper l a r ) = let (x :. xs) = reverseConcat (a :. r) l
                          in ListZipper xs x Nil
...
start ::
  ListZipper a
  -> ListZipper a
start (ListZipper l a r) = let (x :. xs) = reverseConcat l (a :. r)
                           in ListZipper Nil x xs
...
insertPushLeft ::
  ...
  -> ListZipper a
  -> ListZipper a
insertPushLeft x (ListZipper l a r) = ListZipper (a :. l) x r
...
insertPushRight ::
  ...
  -> ListZipper a
  -> ListZipper a
insertPushRight x (ListZipper l a r) = ListZipper l x (a :. r)
...
-- /Tip:/ Use `zipWith`
  (<*>) (ListZipper l1 a1 r1) (ListZipper l2 a2 r2) =
    ListZipper (l1 `apply` l2) (a1 a2) (r1 `apply` r2)
    where
      apply Nil _ = Nil
      apply _ Nil = Nil
...
instance Extend ListZipper where
  (<<=) f z = ListZipper (go moveLeft) (f z) (go moveRight)
    where
      dupApp x = (f x, x)
      go move = unfoldr (\t -> dupApp <$> (toOptional (move t))) z
